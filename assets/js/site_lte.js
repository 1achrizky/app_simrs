$(function () {
  ...
  if( open_site('bo/menu/casemix/pantauan_biaya_ri') ){
    ...
    function my_diff(start=null, end=null, param=null ){
      ...
      if(param == 'tahun') return dur.get('years');
      else if(param == 'bulan') return dur.get('months');
      else if(param == 'hari') return dur.get('days');
      else if(param == 'object') return dur;
      else return '';      
    }
    ...
    function vHistoryDiagnosa(dtjson=null){
      let htmlVal = '';
      for(let i=0; i<dtjson.length; i++){
        htmlVal += 
        '<tr>'+
          '<td>'+dtjson[i].Tanggal+'</td>'+
          '<td>'+dtjson[i].Lokasi+'</td>'+
          '<td>'+dtjson[i].ICD+'</td>'+
          '<td>'+dtjson[i].ICD2+'</td>'+
          '<td>'+dtjson[i].ICD3+'</td>'+
          '<td>'+dtjson[i].ICD4+'</td>'+
    ...
    $("input[name=nobill]").keypress(function (e) { 
      ...
      if (e.which == 13) { //TEKAN ENTER
        ...
        vHistoryDiagnosa(billEnterAll.rs.historyDiagnosa);

        if(biopx.kodePenanggung!='CO051') $('.tblKemkes').hide();
        else $('.tblKemkes').show();
        ...
        if(biopx.StatusDaftar == "RJ" || biopx.StatusDaftar == "UG"){
          if(biopx.StatusDaftar == "UG") $('.menu_igd').show();
          
          $('.menu_rj').show();

          // $('#btnDetailBill_pdf').attr("href",baseUrl()+"ajaxreq/modif_gd_biaya_ri_det_by_billing_pdf_v2/"+nobill);
          $('#btnDetailBill_pdf').attr("href",baseUrl()+"ajaxreq/sep_detbill_pdf/2/"+nobill);
    ...
    $('button[name=btn_insert_historidiag]').click(function(){
      ...
      if(verifikator == ''){
        ...
        console.log(dt);
        if(dt == null) alert('Proses Sukses. Histori diagnosa pasien ditambahkan.');
        else alert('Proses Gagal. Silahkan ulangi.');
    ...
    $('#btn_editulangklaim').click(function(e){
      ...
      alert(js_reedit_claim.metadata.message);
      $('#btn_final_new').show();
      // // JIKA SUKSES
    ...
    $('#btn_final_new').click(function(e){
      ...
      statusRawatLabel = $('#jnsPelayanan').val();
      console.log([nobill, id_histori, date_krs, statusRawatLabel]);
      ...
      if(statusRawatLabel != 'Rawat Jalan'){
      ...
      let send = {
        ...
        Terapi            : $('textarea[name=daftarObatRscmByBill]').val(),
        final_ina   : {
          nomor_sep : $('input[name=nosep]').val(),
          coder_nik : $('#coder_nik').val(),
        },
        ...
        statusRawatLabel    : statusRawatLabel,
        LokasiKode    : biopx.LokasiKode,
      };
      console.log(send);
    ...
    // // stackoverflow
    // // https://stackoverflow.com/questions/70675994/how-to-add-external-data-to-javascript-for-jquery-auto-complete
    ...
    // let r = _ajax_web('GET', url, ''); // your ajax script
    // console.log(r);
    ...
    $('#btn_grouping').click(function(e){
      ...
      //-- SET_CLAIM INACBG
      jenis_rawat = js_sepcbg.response.pesertasep.tktPelayanan;
      let set_claim = 
      {
        "metadata": {
          "method": "set_claim_data",
        ...
        "data": {
          "nomor_sep"     : $('input[name=nosep]').val(),
          "nomor_kartu"   : biopx.Barcode,
          "tgl_masuk"     : biopx.TanggalMasuk+' 08:00:00',
          "tgl_pulang"    : $('input[name=tgl_krs]').val()+' 08:00:00', //"2017-12-01 09:55:00",
          "jenis_rawat"   : jenis_rawat, //  1 = rawat inap, 2 = rawat jalan
      ...
      if(jenis_rawat=='1'){
        set_claim.data['diagnosa_inagrouper'] = diags_val;
        set_claim.data['procedure_inagrouper'] = procs_val;        
      }
      ...
      res_inagr = js_grouping.response_inagrouper;

      // mdc = ['36']
      // drg = ['3600019']
      if(res_inagr.mdc_number=='36'){ $('#mdc').css({'background':'red','color':'white'}); }
      else $('#mdc').css({'background':'white','color':'black'});

      if(res_inagr.drg_code=='3600019'){ $('#drg').css({'background':'red','color':'white'}); }
      else $('#drg').css({'background':'white','color':'black'});

      $('#mdc').text(res_inagr.mdc_description+ ' ('+res_inagr.mdc_number+')');
      $('#drg').text(res_inagr.drg_description+' ('+res_inagr.drg_code+') ');
      ...
      if(js_grouping.metadata.code==200){
        ...
        if( js_grouping.special_cmg_option != undefined){
          for (let c = 0; c < js_grouping.special_cmg_option.length; c++) {
            if(js_grouping.special_cmg_option[c].type == 'Special Procedure') cmgOptLabel = 'Proc';
            else if(js_grouping.special_cmg_option[c].type == 'Special Prosthesis') cmgOptLabel = 'Pros';
            else if(js_grouping.special_cmg_option[c].type == 'Special Investigation') cmgOptLabel = 'Inv';
            else if(js_grouping.special_cmg_option[c].type == 'Special Drug') cmgOptLabel = 'Drug';
            else{}
      ...
      if(cmgOptSend == ''){ // netralisir semua CODE & TARIF SPECIAL_CMG element table
        ...
        if(js_grouping2.metadata.code==200){
          ...
          for (let c = 0; c < js_grouping2.response.special_cmg.length; c++) {
            if(js_grouping2.response.special_cmg[c].type == 'Special Procedure') cmgOptLabel = 'Proc';
            else if(js_grouping2.response.special_cmg[c].type == 'Special Prosthesis') cmgOptLabel = 'Pros';
            else if(js_grouping2.response.special_cmg[c].type == 'Special Investigation') cmgOptLabel = 'Inv';
            else if(js_grouping2.response.special_cmg[c].type == 'Special Drug') cmgOptLabel = 'Drug';
            else{}
  ...
  if( open_site('bo/menu/receptionist/pendaftaranrj/pendaftaran-rjri') ){
    ...
    // console.log(book_plh.norequest);
    // if(book_plh.norequest===undefined)  
    // console.log( 'unde' ); 

    // if(typeof(book_plh.norequest) != "undefined" && book_plh.norequest !== null) {
    //   console.log( 'unde' ); 
    // }
    ...
    //========================== MODAL AMBIL BOOKING ===========================
      ...
      $(document).on('click','table[name=tbl_booking_daftar] tbody tr td button[name=btn_mdl_plh_px_book]', function(){
        ...
        noantrii = book_plh.norequest;
        console.log('noantrii', book_plh.norequest);
        ...
        if(cek_px_masuk.ws_rs.get_pxrs.length == 0){
          ...
          if(rjk_multi != null){
            if(rjk_multi.metaData.code == 200){
              ...
              $("input[name=kelas_bpjs]").val(px_bpjs.response.peserta.hakKelas.keterangan); // px_bpjs.response.peserta.noKartu
    ...
    $(document).on('click','button[name=btn_lewati_cetak_kartu]', function(){
      ...
      if(get_penanggung_cm_kode == "CO031"){
        ...
        let arr_px_bpjs_POST = {
          ...
          "kelas_bpjs"    : px_bpjs.response.peserta.hakKelas.keterangan, //px_bpjs.response.peserta.noKartu, 
          "asal_rujukan" : "", 
    ...
    let createSep = true;
    $('input[name="chk_notCreateSep"]').on('ifChanged', function(e){
      createSep = !(e.target.checked);
      console.log('createSep', createSep);
    });
      ...
      // //++++++++++++++++++++ create_sep ++++++++++++++++++++++++
          ...
          sep_req =                                                     
          {
            "request": {
              "t_sep": {
                  "noKartu": get_noka,
                  "tglSep": $('input[name=tglsep]').val(), // moment().format('YYYY-MM-DD'),
        ...
        console.log(sep_res);
        let nosepp = sep_res.response.sep.noSep;
        $("input[name=get_nosep_temp]").val(nosepp); 
    });
    ...
    $('#btn_daftar').click(function(){
      ...
      if(penanggung_plh.bpjs){
          ...
          if(createSep){
            ...
            // console.log($('input[name=TglLahir]').val()); return false;
            sep_res = 
            {
                "metaData": {
                    "code": "200",
      ...
      if(_FL_ambil_px_book==0)
        noantrii = exe.response.xrec.data.data_paket.jpost_insert_reg_cm.generate.antri["3d"];
      ...
      // START PRINT di PENDAFTARAN RJ
        ...
        if(_FL_daftar_ugd == 0){ // jika bukan DAFTAR UGD
          jpost_cetak_noantrian = {
            ...
            no_antrian : noantrii, // book_plh.norequest, // noreqq,
        ...
        //++++++++++++++++++++++ print tracer +++++++++++++++++++++++++++++++++++++
        let prb = (penanggung_plh.bpjs)? sep_res.response.sep.informasi.prolanisPRB : '';
        console.log(prb);
        ...
        // if(_FL_daftar_ugd == 0){ // SELAIN IGD
          jpost_cetak_tracer = {
            ...
            no_antrian: noantrii, // book_plh.norequest, // exe.response.xrec.data.data_paket.jpost_insert_reg_cm.generate.antri["3d"],// noantrian_ready_3digit,
      ...
      // //+++++++++++++++++++++++ print preview cetak_resume_sep ++++++++++++++++++++++++
        ...
        // if(penanggung_plh.bpjs || penanggung_plh.NoAnggota == "KEMENKES"){
          ...
          jpost_cetak_resume_sep = {
              ...
              prb         : prb,
              nourut      : noantrii, // exe.response.xrec.data.data_paket.jpost_insert_reg_cm.generate.antri["3d"],
              tglRujukan  : get_tglRujukan,
          };
    ...
    $('#btnCariNokaBpjs').click(function(e){
      e.preventDefault();
      let nokaNew     = $('input[name=noka_bpjs]').val();
      console.log(nokaNew);
  ...
  if( open_site('bo/menu/receptionist/pendaftaranrj/pengajuan-sep') ){
    $('.datepicker').datepicker({
      autoclose : true,
      format    : 'yyyy-mm-dd'
    });
    ...
    $("input[name=noKartu]").keypress(function (e) { //TEKAN ENTER
      ...
      // console.log(nosep);
      if (e.which == 13) {
        let cari_px = _ajax_bpjs("GET", "peserta_cari_get", {noKartu:noKartu});
        console.log(cari_px);
        // nama = cari_px.response.peserta.nama;
        // tgl_sep = cari_px.response.tglSep;
        $('input[name=nama]').val(cari_px.response.peserta.nama);    
        // $('input[name=tgl_sep]').val(tgl_sep);    
      }
    });
    
    $('#btn_aproval').click(function(e){
      e.preventDefault();
      P_aproval = {
        "request": {
            "t_sep": {
              ...
              "tglSep": $('input[name=tglSep]').val(),
              "jnsPelayanan": $('select[name=jnsPelayanan]').val(),
              "jnsPengajuan" : $('select[name=jnsPengajuan]').val(),
              "keterangan": $('input[name=keterangan]').val(),
              "user": "16141"
            }
        }
      };
      console.log(P_aproval);
      let upd_ap = _ajax_bpjs("POST", "aprovalSEP", P_aproval);
      console.log(upd_ap);
      if(upd_ap.metaData.code == 200){        
        alert(upd_ap.response);
        reload();
      }else{
        alert("GAGAL. ("+upd_ap.metaData.code+") "+upd_ap.metaData.message);
        return false;
      }  
    });
    ...
    $('#btn_ok').click(function(e){
      ...
      let upd = _ajax_bpjs("POST", "pengajuanSEP", P);
      console.log(upd);
      // return false;
      ...
      if(upd.metaData.code == 200){        
        ...
        alert("GAGAL. ("+upd.metaData.code+") "+upd.metaData.message);
      }
  ...
  if( open_site('bo/menu/receptionist/laporan/lap-daftarrj') ){
    ...
    let js_sel = '';
      ...
      $(document).on('click','table[name=tbl_lap_daftarrj] tbody tr td a', function(e){
        ...
        $('table[name=tbl_detail_pasien] tr td input[name=in_nosep]').val(js_sel.nosep);
      ...
      $('#btn_upd_sep').click(function(e){
        e.preventDefault();
        console.log(js_sel.nobill, $('input[name=in_nosep]').val());

        // return false;
        exe_update = _db_update({NoBill:js_sel.nobill}, 'fotrdaftar', {nosep: $('input[name=in_nosep]').val()});
        console.log(exe_update);
        if(exe_update==null) alert('Data berhasil diupdate.');
        return false;
      });
      ...
      $(document).on('click','button[name=btn_cetak_antrian_skdp_langsung]', function(e){
        e.preventDefault();
        ...
        // // REPAIRING_RIZ
        let jpost = { url: _ADDR, button_id:$(this).attr("name"), norm_noka:'norm', nomor:js[get_id].norm, date:tgl};
      ...
      $(document).on('click','button[name=btn_cetak_resume_sep]', function(e){
        ...
        if(get_sep == 0 || get_sep == ''){
          ...
          jpost_cetak_resume_sep = {
              ...
              alamat  : js_sel.alamat,
              // tglSep  : moment().format('YYYY-MM-DD'),
              tglSep  : js_sel.tanggalmasuk, // jsObj_px_cm.datajs[0].TanggalMasuk, //?
              noKartu : '',
              nama    : '',
              nama_cm : js_sel.nama,
              tglLahir: js_sel.tgllahir, // jsObj_px_cm.datajs[0].TglLahir, //?
              poli    : '',
              diagnosa: '',
              catatan : '',
              ...
              billing     : js_sel.nobill,
              lokasi_ket  : js_sel.lokasi, //??? spesialis_pilih, //?
              nama_dokter : js_sel.dokter_nama, //??? namaDokter_pilih, //?
              penanggung_cm: js_sel.penanggung_ket, //??? get_penanggung_cm_nama, //?
              prb         : '',
      ...
      $(document).on('click','#btn_cetak_antrian_skdp_1', function(e){
        ...
        let jpost = { url: _ADDR, button_id:$(this).attr("id"), norm_noka:'norm', nomor: js[get_id].norm, };
        let wsprinter = _ajax_web("POST", baseUrl()+"print_termal/cetak_skdp_antrian", jpost);
        console.log(wsprinter);            
      });
