$(function () {
  ...
  if( open_site('daftarmandiri/px_cetak_antrian') ){
    ...
    $('#btn_cetak_antrian_skdp').click(function(e){
      ...
      console.log([sel_cari_by, in_cari_by, date]);
        ...
        let js_sel = _ajax("GET", "cetak_skdp_antrian/"+sel_cari_by+"/"+in_cari_by, "");
        console.log(js_sel);      
        
        if(js_sel==undefined){
          alert("Data tidak ditemukan. Pendaftaran pasien belum diproses.");
        }else{
          ...
          console.log(jpost_cetak_skdp);
          let js_skdp = _ajax("POST", "cetak_skdp_langsung_termal", jpost_cetak_skdp);
          console.log(js_skdp);
        }
  ...
  if( open_site('daftarmandiri/px_cetak_antrian_rc') ){
    ...
    let js = _ajax("GET", "db/m_daftarmandiri/select_nomor_antridaftar_max/"+date, "");
    console.log(js);
    let number = 1;
    
    if(js==null){ // antrian pertama hari=$date
      $("#number").text(number);
    }else{
      number = parseInt(js.nomor)+1;
      $("#number").text(number);
    }
    ...
    $('#btn_cetak_antrian_rc').click(function(e){
      ...
      // let nomor = $("#number").text();
      js = _ajax("GET", "db/m_daftarmandiri/select_nomor_antridaftar_max/"+date, "");
      console.log(js);
      if(js!=null){
        number = parseInt(js.nomor)+1;
      }
      ...
      js = _ajax("GET", "termal_nomor_antrian_daftar/"+insert.nomor+"/"+insert.date+" "+insert.mulai, "");
      console.log(js);

      let jInsert = _db_insert('antridaftar', insert);
      console.log(jInsert);
  ...
  if( open_site('bo/menu/casemix/pantauan_biaya_ri') ){
    ...
    // $('#btn_grouping2').click(function(e){
    //   e.preventDefault();
    //   cmgOptSelectedAll = ['t', 'q', '', 'W'];
    //   cmgOptSelectedAll = $.grep(cmgOptSelectedAll, function(check) {
    //     return (check != '');
    //   });
    //   console.log(cmgOptSelectedAll);
  ...
  if( open_site('bo/menu/casemix/laporan_pasien_ri') ){
    ...
    $('button[name=btn_tgl_px_ri]').click(function(e){
      ...
      dt = _ajax('GET', 'select_laporan_px_ri_by_daterange/'+date+'/'+date_end, '');
      console.log(dt);
    ...
    $('button[name=btn_dl_excel_px_ri]').click(function(e){
      ...
      console.log(date);
      // $.redirect(baseUrl()+'ajaxreq/download_xls_laporan_px_ri?date='+date); //ndak bisa
  ...
  if( open_site('bo/menu/receptionist/pendaftaranrj/daftarbooking') ){
    ...
    let dt_book_web = _ajax_web('GET', 'https://citramedika.com/daftaronline/model-pdo.php', {kode:'select_booking_all'});
        ...
        if(curl_TX.status == 'SUKSES'){
          ...
          if(btn_pilih_dari_web){
            ...
            console.log(dt);
              ...
              if(chk_tracer_rc){                
                let ct_tr1 = _ajax("POST", "cetak_tracer_v3/label_printer/termal_nomor_antrian", jpost_cetak_tracer);
                console.log(ct_tr1);
                
                let ct_tr2 = _ajax("POST", "cetak_tracer_v3/label_printer/termal_nomor_antrian", jpost_cetak_tracer);
                console.log(ct_tr2);
              }else{
                let ct_tr1 = _ajax("POST", "cetak_tracer_v3", jpost_cetak_tracer);
                console.log(ct_tr1);
                
                let ct_tr2 = _ajax("POST", "cetak_tracer_v3", jpost_cetak_tracer);
                console.log(ct_tr2);
              }
    ...
    // // BARU 2020.01.16
    // $('#btn_form_daftar_online_n').click(function(e){
    //   e.preventDefault();
    ...
    //   if(_bpjs_syarat_rjk == 1){
    //     // let curl_TX = send_form_daftar_online_CURL_TX(jPost);
    //     let curl_TX = _ajax_web("POST", baseUrl+"ajaxreq/send_form_daftar_online_CURL_TX", jPost);
    //     console.log(curl_TX);
    //     if(curl_TX.status == 'SUKSES'){
    //       message = 'Pasien bernama: '+curl_TX.nama+'.\nBerhasil didaftarkan.';
    ...
    //           NoBill    : "BOOKING",
    //           NoRM      : get_norm,
    //           no_antrian: curl_TX.norequest, // exe.response.generate.antri["3d"],
    //           // Nama      : rs.mp.Nama, //$('span[name=pasienRscm_nama]').text(),
    //           // TglLahir  : rs.mp.TglLahir, //pasien_cm_Obj.datajs[0].TglLahir,
    //           // Sex       : rs.mp.Sex, //$('span[name=pasienRscm_jk]').text(),
    //           // Alamat    : rs.mp.Alamat, //$('span[name=pasienRscm_alamat]').text(),
    //           klinik    : lokasiket, // rs.dokter.spesialis,
    //           dokter    : $('select[name=sel_dokter]').val(), // rs.dokter.nama,
    //           user      : _user_daftar,
    //           ket       : "", // rs.ket_daftar, //???
    //           // umur      : get_umur_fx(rs.mp.TglLahir, 'tahun'),
    //           // st_px_baru_lama : exe.response.jpost_insert_reg_cm.data.StatusBL,//st_px_baru_lama //???
    //           penanggung_cm: get_penanggung_cm_nama,
    //         };
    //         console.log(jpost_cetak_tracer);

    //         _ajax_web("POST", baseUrl()+"main/direct_print/termal_tracer_booking/"+get_norm, jpost_cetak_tracer);
    ...
    $('#btn_daftar').click(function(){
      ...
      // START PRINT di PENDAFTARAN RJ
        ...
        */
        ...
        if(_FL_ambil_px_book == 0){
          // cetak_skdp_langsung(jpost_cetak_skdp);
          let js_skdp = _ajax("POST", "cetak_skdp_langsung_termal/1", jpost_cetak_skdp);
          console.log(js_skdp);
        }
        ...
        // if(_FL_daftar_ugd == 0){ // SELAIN IGD
          ...
          if(_FL_ambil_px_book == 0){ 
            if(chk_tracer_rc){
              ...
              let ct_tr2 = _ajax("POST", "cetak_tracer_v3/label_printer/termal_nomor_antrian", jpost_cetak_tracer);
              console.log(ct_tr2);
            }else{ // JIKA DEFAULT, CETAK TRACER KE RM
              let ct_tr1 = _ajax("POST", "cetak_tracer_v3", jpost_cetak_tracer);
              console.log(ct_tr1);
              console.log(">> CETAK TRACER 1");
  ...
  if( open_site('bo/menu/receptionist/laporan/lap-daftarrj') ){
    ...
    let js_sel = '';
      ...
      $(document).on('click','table[name=tbl_lap_daftarrj] tbody tr td button[name=btn_cetak_antrian_skdp_langsung]', function(e){
        ...
        js_sel = _ajax("GET", "db/m_daftarmandiri/laporan_pendaftaran_px_soft_by_bill/"+js[get_id]["segment"]+"/"+js[get_id]["nobill"]+"/"+js[get_id]["kodelokasi"], "");
        if(js_sel.length>0) js_sel = js_sel[0];
        console.log(js_sel);
        ...
        console.log(js_antrian);
        // cetak_nomor_antrian(jpost_cetak_noantrian);
        // ini hasil copy script lama tidak dipakai. hapus fx bila tidak dipakai
      ...
      $(document).on('click','button[name=btn_cetak_antrian]', function(e){
        e.preventDefault();
        let jpost_cetak_noantrian = {
              billing   : js_sel.nobill,
              no_antrian: js_sel.nourut,
      ...
      $(document).on('click','button[name=btn_cetak_skdp]', function(e){
        e.preventDefault();
        let jpost_cetak_skdp = {
            billing : js_sel.nobill,
            noskdp  : js_sel.noskdp,
      ...
      $(document).on('click','button[name=btn_cetak_tracer]', function(e){
        e.preventDefault();
        console.log(">> BTN/CETAK TRACER");
      ...
      $(document).on('click','button[name=btn_cetak_tracer_igd]', function(e){
        e.preventDefault();
        ...
        let ct_tr1 = _ajax("POST", "cetak_tracer_v3/rc", jpost_cetak_tracer);
        console.log(ct_tr1);
  ...
  if( open_site('bo/menu/receptionist/laporan/lap-booking') ){
    ...
    $(document).on('click','#tbl_book_tmp tbody tr td .btnPrintIdList', function(e){
      ...
      let jpost_cetak_tracer = {
        NoBill    : "BOOK"+ moment(js_sel.date).add('days', 1).format('YYYY-MM-DD'), //tglbook??
        NoRM      : js_sel.norm,
        ...
        user      : _user_logged_in,
        ket       : js_sel.keterangan,
        umur      : get_umur_fx_new(js_sel.TglLahir).tahun, // js_sel.umur, //??
        st_px_baru_lama : 'LAMA', // js_sel.statuspasien, //??
        penanggung_cm   : js_sel.penanggungket, 
        pdp       : pdp,
      };
      console.log(jpost_cetak_tracer);
  ...
  if( open_site('bo/menu/manajemen/dokter-spesialis/tarif-sp-ina-rs') ){
    ...
    $('#btn_dl_excel').click(function(e){
      ...
      // window.open(baseUrl()+'ajaxreq/select_laporan_px_ri_by_daterange/'+$('#in_datestart').val()+"/"+$('#in_dateend').val(), '');
  ...
  if( open_site('bo/menu/it/setting/printer') ){
    ...
    $('#sel_label_printer').on('change', function(){
      ...
      // filteredObjn.value = String.raw`\\192.168.1.104\TM-P230C Series`.split('\\').join('/');
    ...
    $('#btn_update_printer').click(function(e){
      ...
      // $('#sel option').removeAttr("selected");
      // $('#sel option[value="aa"]').attr("selected", true);
      // console.log($('#sel').val());
      
      // $('#sel option').removeAttr("selected");
      // // $('#sel option[value="\\192.168.1.104\TM-P230C Series"]').attr("selected", true);
      // $('#sel option[value="\\\\192.168.1.104\\TM-P230C Series"]').attr("selected", true);
      // console.log($('#sel').val());
      ...
      // let str = "\\192.168.1.104\TM-P230C Series";
      ...
      // $('#sel option[value="'+str+'"]').attr("selected", true);
      // alert(str);
  ...
  if( open_site('bo/menu/rm/asesmen-rehab-medis') ){
    ...
    $(document).on('click','#tbl_mdl_store tbody tr td button', function(e){
      ...
      let P = {
        norm : norm,
        idasesmen : id_sel,
        urut         : tdkn_sel.length+1, // i+1, //?
        nobill       : js_selected['nobill'],
        tindakan     : js_selected['detail_tx'],
        kodeTindakan : js_selected['detailkode_tx'],
      };
      console.log(P);

      el += '<tr data-id="'+i+'">'+
            '<td>'+(i+1)+'</td>'+
            '<td>'+js_selected['nobill']+'</td>'+
            '<td>'+js_selected['detail_tx']+'</td>'+
            '<td><button class="btn-danger" data-id="'+i+'">x</button></td>'+
          '</tr>';        
      
      $("#tbl_li_sel").append(el);
      // return false;

      ins = _ajax_web("POST", baseUrl()+"rm/insert_detail_tindakan_rehab", P);
      console.log(ins);
    ...
    $("#btn_cetak_asesmen").click(function(){ 
      if(norm == ''){ alert('NORM belum diisi.'); return false; }
      id_sel = $("select[name=paket]").find('option:selected').val();
      if(id_sel == ''){ alert('Paket belum dipilih.'); return false; }

      popup_print_url('asesmen-rehab-medis/'+norm+'/'+id_sel);
      return 0;      
    });
