...
class Print_kertas {
	// ISI DENGAN printer>properties>sharing>{SHARE NAME}
	private $printer = array(
		...
		'termal_tracer_rc' 	 		=> '\\\\192.168.1.93\\printerantrian',
		'termal_nomor_antrian_daftar' => '\\\\192.168.1.93\\printerantrian',
		...
		'cetak_skdp_langsung_termal'=> '\\\\192.168.1.93\\printerantrian',
		'termal_nomor_antrian_new'=> '\\\\192.168.1.93\\printerantrian',
				
		
		'cetak_skdp_langsung_termal_1'=> '\\\\192.168.1.104\\Code Soft 32xx Series',//?
		'termal_nomor_antrian_new_1'=> '\\\\192.168.1.104\\Code Soft 32xx Series',//?
	);
	...
	public function printer_db_default($label=null){
		// PRINTER DB
		$CI =& get_instance();
		$CI->load->model('m_it');
		
		$prin = $CI->m_it->select_printer();

		$pr = [];
		for ($i=0; $i < count($prin['db']); $i++) 
			$pr[ $prin['db'][$i]['name'] ] = $prin['db'][$i]['value'];
	...
	public function termal_nomor_antrian_new($data=null, $select_printer=null){
		...
		if($select_printer==null){ // RM
			$printer_name_selected = 'termal_nomor_antrian_new';
		}else{
			$printer_name_selected = 'termal_nomor_antrian_new_'.$select_printer;
		}
		$alamat = $this->printer[$printer_name_selected];
		// /* write the text to the print job */  
		// var_dump($printer);
		...
		printer_set_option($handle, PRINTER_SCALE, 50);
		//FIX//$handle = printer_open();
		printer_start_doc($handle, $printer_name_selected);
		printer_start_page($handle);
	...
	public function termal_tracer($nama_printer,$data=null){
		...
		printer_select_font($handle, $font);
		$text = "PRB:".$data['prb'];
		printer_draw_text($handle, $text, $margin_left_page, 120+(30*12)+15);	
		printer_delete_font($font);
	...
	public function termal_tracer_v3($data=null, $lokasi=null, $label_printer=null){
		...
		if($lokasi == 'rc'){
			$alamat = $this->printer['termal_tracer_rc']; //tracer rc // ERROR. tercetak di RM, padahal sudah di setting di rc
			// $alamat = $this->printer['termal_nomor_antrian_new']; //tracer rc
		}else if($lokasi == "label_printer"){
			$alamat = $this->printer[$label_printer]; //tracer rc
		...
		// ($handle)? $x="yes":$x="no";
		// echo json_encode($x);
		// echo json_encode(["x" => $handle]);
		// return $handle;

		// TESTING
		// echo "<pre>",print_r($handle),"</pre>"; exit;
		// echo json_encode($handle); // BISA
	...
	public function cetak_skdp_langsung_termal($data=null, $select_printer=null){
		header('Access-Control-Allow-Origin: *'); 
		header('Access-Control-Allow-Methods: GET, PUT, POST, DELETE, OPTIONS');

		// $alamat = $this->printer['cetak_skdp_langsung_termal'];
		if($select_printer==null){
			$printer_name_selected = 'cetak_skdp_langsung_termal';
		}else{
			$printer_name_selected = 'cetak_skdp_langsung_termal_'.$select_printer;
		}
		$alamat = $this->printer[$printer_name_selected];
